...
class Home extends CI_Controller {
			...
			if($ins > 0)
			{
				...
				if(@sizeOf($allpages) > 0)
				{
					for($s=0;$s<sizeOf($allpages);$s++)
					{
						...
						{
							if($allpages[$s]->pAlias == 'portfolio')
							{
							...
							{
								$str .= "\n"."&#36;route['".@$this->clean($allpages[$s]->seoCustomURL)."']='home/blogs';";
							}
	...
	public function work()
	{
		...
		{
			$data['menu'] = 'work';
			$data['items'] = $this->sql->getTableAllDataOrder("works","orderpos","ASC");
			$this->load->view('header',$data);			
	...
	public function updateWorksOrder()
	{
		if($this->session->userdata("is_logged_in") != 1)
		{
			redirect(base_url());
		...
		else
		{
			...
			if(@sizeOf($rowids) > 0)
			{
				for($r=0;$r<sizeOf($rowids);$r++)
				{
					$order=$this->input->post("orderpos_".$rowids[$r]);
					$params=array(
						'orderpos' => $order,
						'updateDate' => @date("Y-m-d H:i:s")
					);
					$ins = $this->sql->updateItems("works",$params,array('id' => $rowids[$r]));
					if($ins > 0)
					{
						$set = 1;
					}
